...
type FileUploadProps = {
  ...
  clearMaterial?: (index?: number) => void;
  className?: string;
  maxFileSize?: number;
  description?: string;
  multiple?: boolean;
...
const FileUpload: React.FC<FileUploadProps> = ({
  ...
  const handleFileValidation = (files: FileList) => {
    if (maxFileSize && Array.from(files).some(file => file.size > maxFileSize)) {
      toast.error('One or more files exceed the maximum size limit.');
      return false;
    }
    return true;
  };
  ...
  const handleDrop = (e: React.DragEvent<HTMLLabelElement>) => {
    ...
    if (droppedFiles.length > 0 && handleFileValidation(droppedFiles)) {
      const event = {
        target: {
          files: droppedFiles,
        },
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && handleFileValidation(files)) {
      handleFileChange(e);
    }
  };
  ...
  const renderUploadUI = () => (
    <>
        ...
        <div className="mt-4 space-y-3">
          {Object.entries(groupFormats(formats)).map(([group, formats]) => (
            <div key={group} className="text-center">
              <span className="text-sm font-bold text-gray-600 block">{t(`course_creation.${group?.toLowerCase()}`)}</span>
      ...
      {description && (
        <p className="text-sm text-gray-500 mt-2 px-6 text-center max-w-md">{description}</p>
      )}
    </>
  );
    ...
    if (!multiple && file) {
      return (
        <div className="flex flex-col items-center justify-center p-6">
          ...
          <div className="flex items-center mt-4 bg-gray-50 rounded-lg p-3">
            <span className="text-gray-700 font-medium">{file.name}</span>
            <button
              className={
                disabled
                  ? 'text-gray-500'
                  : 'ml-2 p-1 rounded-full hover:bg-gray-200 transition-colors'
              }
              onClick={e => {
                e.preventDefault();
                clearFile();
  ...
  return (
    <div className={`w-full ${className}`}>
      <label
        htmlFor="file-upload"
        className={`flex flex-col items-center justify-center w-full min-h-64 rounded-lg transition-all duration-200 py-2 px-3 ${
          dragActive
            ? 'bg-blue-50 border-blue-400 border-2 shadow-lg'
            : `bg-background border-2 border-dashed border-gray-300`
        } ${disabled ? '' : 'hover:border-blue-300 hover:bg-gray-50 cursor-pointer'}`}
      ...
      {multiple && files.length + materials.length > 0 && (
